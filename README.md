...
> **💡 Running in CodeSandbox?** See [CODESANDBOX.md](./CODESANDBOX.md) for online environment setup
...
The project uses Expo's default Metro configuration (`metro.config.js`). Storybook is loaded at runtime via the toggle button in `app/_layout.tsx`, which avoids ESM/CommonJS conflicts and works in CodeSandbox and other online environments.

Story discovery is handled by `storybook.requires.ts` (auto-generated by `npm run storybook:update`).
